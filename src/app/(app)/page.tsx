  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <Stagger className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <FadeIn><Kpi label="Leads no período" value={r.totalLeads.toLocaleString("pt-BR")} accent="navy" /></FadeIn>
        <FadeIn><Kpi label="Consultas realizadas" value={r.consultas.toLocaleString("pt-BR")} sub={brl(r.valorRealizado)} accent="green" /></FadeIn>
        <FadeIn><Kpi label="Ticket médio" value={brl(r.ticket)} accent="teal" /></FadeIn>
        <FadeIn><Kpi label="Taxa de conversão" value={`${r.conversao.toFixed(1)}%`} sub={`${r.emAberto} em aberto`} accent="green" /></FadeIn>
      </Stagger>

      <Stagger className="mt-4 grid grid-cols-2 gap-4 lg:grid-cols-4">
        <FadeIn><Kpi label="Leads perdidos" value={r.perdidos.toLocaleString("pt-BR")} accent="navy" /></FadeIn>
        <FadeIn><Kpi label="Taxa de perda" value={`${r.taxaPerda.toFixed(1)}%`} accent="navy" /></FadeIn>
        <FadeIn className="lg:col-span-2">
          <Card className="bg-gradient-to-br from-ptd-navy to-ptd-teal text-white">
            <div className="text-xs font-semibold uppercase tracking-wide text-white/70">Oportunidade perdida (estimada)</div>
            <div className="mt-2 text-3xl font-extrabold tabular-nums">{brl(r.oportunidadePerdida)}</div>
            <div className="mt-1 text-xs text-white/70">{r.perdidos} leads perdidos × ticket médio {brl(r.ticket)}</div>
          </Card>
        </FadeIn>
      </Stagger>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-5">
        ...
        <FadeIn className="lg:col-span-2">
          <Card>
            <div className="mb-3 text-sm font-semibold text-ptd-ink">Origem dos leads</div>
              ...
              {consultas.map((c) => (
                <div key={c.lead_id} className="flex items-center justify-between py-2.5">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-ptd-muted">{[c.unidade, c.origem].filter(Boolean).join(" · ") || c.telefone || ""}</div>
                  </div>
                  <div className="flex items-center gap-3">
                    {(c.valor_consulta || c.valor) > 0 && <Badge tone="green">{brl(c.valor_consulta || c.valor)}</Badge>}
                    <span className="text-xs text-ptd-muted">{c.criado_em ? new Date(c.criado_em).toLocaleDateString("pt-BR") : ""}</span>
                  </div>
                </div>
              ))}
            </div>
